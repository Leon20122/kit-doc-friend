...
export function MarcoTeorico() {
  const [opAmpText, setOpAmpText] = useState<string>('');

  return (
    <div className="animate-fade-in">
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
          <span>📖</span> Marco Teórico
        </h1>
        <p className="text-sm text-muted-foreground mt-1">
          Fundamentos teóricos del amplificador operacional y transistores BJT.
        </p>
      </div>
      ...
      <p className="text-sm text-foreground/80 mb-4 leading-relaxed">
        Un amplificador operacional (op-amp) es un amplificador electrónico de alta ganancia con
        entrada diferencial y, normalmente, una salida única. Es uno de los bloques fundamentales
        de la electrónica analógica y se utiliza ampliamente en aplicaciones de procesamiento de señales.
      </p>

      {/* Características Ideales vs Reales — textarea editable */}
      ...
      <div className="bg-card rounded-xl border border-border p-4 mb-6">
        <textarea
          value={opAmpText}
          onChange={e => setOpAmpText(e.target.value)}
          placeholder={
            'Describe aquí libremente las características del op-amp ideal y real...\n\nEjemplo:\n— Ideal: Ganancia infinita, Zin infinita, Zout = 0...\n— Real: Ganancia 10⁵–10⁶ V/V, Zin 1MΩ–1TΩ...'
          }
          rows={10}
          className="w-full bg-transparent text-sm text-foreground/90 placeholder:text-muted-foreground/50 outline-none resize-y leading-relaxed"
        />
      </div>
      ...
      <p className="text-sm text-foreground/80 mb-4">
        Un amplificador operacional discreto típico se compone de tres etapas principales conectadas en cascada:
      </p>
      ...
      <ToggleBlock title="Etapa Diferencial (Entrada)">
        <p className="text-sm text-foreground/80 mb-2">
          <strong>Función:</strong> Amplifica la diferencia entre las dos señales de entrada (V+ y V-)
          mientras rechaza señales comunes.
        </p>
        <p className="text-sm text-foreground/80 mb-3">
          <strong>Componentes:</strong> Par de transistores NPN (Q1, Q2) con fuente de corriente (Iee).
        </p>
        <h4 className="text-sm font-semibold text-foreground mb-2">Ecuaciones Clave</h4>
        <div className="space-y-1">
          {[
            'Ad = gm × Rc',
            'gm = Ic / VT   (donde VT ≈ 26mV a 27°C)',
            'CMRR = Ad / Acm = 2 × gm × Ree',
          ].map((eq, i) => (
            <div key={i} className="bg-secondary/50 rounded px-3 py-1.5 text-sm font-mono text-primary">
              {eq}
            </div>
          ))}
        </div>
      ...
      <ToggleBlock title="Etapa de Ganancia (Intermedia)">
        <p className="text-sm text-foreground/80 mb-2">
          <strong>Función:</strong> Proporciona ganancia de voltaje adicional. Convierte la salida
          diferencial en single-ended.
        </p>
        ...
        <div className="bg-secondary/50 rounded px-3 py-1.5 text-sm font-mono text-primary">
          Av_etapa2 = -gm × (Ro ∥ ro)
        </div>
      </ToggleBlock>
      ...
      <ToggleBlock title="Etapa de Salida Push-Pull">
        <p className="text-sm text-foreground/80 mb-2">
          <strong>Función:</strong> Proporciona baja impedancia de salida y capacidad de entregar
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-secondary/50">
              {['Modo', 'Juntura B-E', 'Juntura B-C', 'Uso'].map(h => (
          ...
          <tbody className="divide-y divide-border">
            {[
              ...
              <tr key={i} className="hover:bg-secondary/30">
                ...
                <td className="px-3 py-2 text-foreground/90">{be}</td>
                <td className="px-3 py-2 text-foreground/90">{bc}</td>
                <td className="px-3 py-2 text-foreground/90">{u}</td>
              </tr>
            ))}
          </tbody>
        </table>
